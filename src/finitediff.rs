...
pub fn forward_hessian_nograd_vec_f64(p: &Vec<f64>, op: &Fn(&Vec<f64>) -> f64) -> Vec<Vec<f64>> {
    let fx = (op)(p);
    let n = p.len();
...
mod tests {
    ...
    fn test_forward_hessian_vec_prod_vec_f64() {
        ...
        let hessian = forward_hessian_vec_prod_vec_f64(&p, &|d| d.forward_diff(&op), &x);
    ...
    fn test_forward_hessian_vec_prod_ndarray_f64() {
        ...
        let hessian = forward_hessian_vec_prod_ndarray_f64(&p, &|d| d.forward_diff(&op), &x);
        let res = vec![0.0, 6.0, 10.0, 18.0];
        // println!("hessian:\n{:#?}", hessian);
        // println!("diff:\n{:#?}", diff);
        (0..4)
    ...
    fn test_central_hessian_vec_prod_vec_f64() {
        ...
        let p = vec![1.0f64, 1.0, 1.0, 1.0];
        let x = vec![2.0, 3.0, 4.0, 5.0];
        let hessian = central_hessian_vec_prod_vec_f64(&p, &|d| d.forward_diff(&op), &x);
        let res = vec![0.0, 6.0, 10.0, 18.0];
        // println!("hessian:\n{:#?}", hessian);
        // println!("diff:\n{:#?}", diff);
        (0..4)
    ...
    fn test_central_hessian_vec_prod_ndarray_f64() {
        ...
        let p = ndarray::Array1::from_vec(vec![1.0f64, 1.0, 1.0, 1.0]);
        let x = ndarray::Array1::from_vec(vec![2.0, 3.0, 4.0, 5.0]);
        let hessian = central_hessian_vec_prod_ndarray_f64(&p, &|d| d.forward_diff(&op), &x);
        let res = vec![0.0, 6.0, 10.0, 18.0];
        // println!("hessian:\n{:#?}", hessian);
        // println!("diff:\n{:#?}", diff);
        (0..4)
    ...
    fn test_forward_hessian_nograd_vec_f64() {
        let op = |x: &Vec<f64>| x[0] + x[1].powi(2) + x[2] * x[3].powi(2);
        let p = vec![1.0f64, 1.0, 1.0, 1.0];
        let hessian = forward_hessian_nograd_vec_f64(&p, &op);
        let res = vec![
            vec![0.0, 0.0, 0.0, 0.0],
            vec![0.0, 2.0, 0.0, 0.0],
            vec![0.0, 0.0, 0.0, 2.0],
    ...
    fn test_forward_hessian_nograd_sparse_ndarray_f64() {
        ...
        let p = ndarray::Array1::from_vec(vec![1.0f64, 1.0, 1.0, 1.0]);
        let indices = vec![(1, 1), (2, 3), (3, 3)];
        let hessian = forward_hessian_nograd_sparse_ndarray_f64(&p, &op, indices);
        let res = vec![
            vec![0.0, 0.0, 0.0, 0.0],
            vec![0.0, 2.0, 0.0, 0.0],
            vec![0.0, 0.0, 0.0, 2.0],
